window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var OpenConfigCard_module_css_default = {
			...
			"buttonDanger": "fRws-W_buttonDanger",
			"card": "fRws-W_card",
			...
			"hint": "fRws-W_hint",
			"input": "fRws-W_input",
			"mediaEmpty": "fRws-W_mediaEmpty",
			"mediaKind": "fRws-W_mediaKind",
			"mediaList": "fRws-W_mediaList",
			"mediaName": "fRws-W_mediaName",
			"mediaRow": "fRws-W_mediaRow",
			"name": "fRws-W_name",
			"path": "fRws-W_path",
		...
		function OpenConfigCard() {
			...
			const [parseError, setParseError] = (0, react.useState)(false);
			...
			const [videoPath, setVideoPath] = (0, react.useState)("");
			...
			const refresh = () => {
				...
				fetch("/synthwave-theme-config").then((res) => res.json()).then((data) => {
					if (data && data.sources) {
						setVideoPath(typeof data.sources.video === "string" ? data.sources.video : "");
						setImagePaths(Array.isArray(data.sources.images) ? data.sources.images.filter((p) => typeof p === "string") : []);
					}
				}).catch(() => {});
			};
			(0, react.useEffect)(() => {
			...
			const removeVideo = () => {
				...
				setRemoving(true);
				setHint("正在移除视频…");
				jsonPost("/synthwave-theme-config/remove", {
					kind: "video",
					value: ""
				}).then((result) => {
					if (result && result.ok) {
						setHint("已移除视频，硬刷新页面生效");
						refresh();
					} else setHint("移除失败：" + String(result && result.error || "未知错误"));
				}).catch(() => setHint("移除失败")).finally(() => setRemoving(false));
			};
			...
			return (0, react.createElement)("li", { className: expanded ? `${OpenConfigCard_module_css_default.card} ${OpenConfigCard_module_css_default.cardOpen}` : OpenConfigCard_module_css_default.card }, (0, react.createElement)("button", {
				...
				className: OpenConfigCard_module_css_default.button,
				onClick: copy
			}, "复制路径")), parseError ? (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.warning }, "⚠ 配置文件解析失败：当前已临时套用默认配置（未修改原文件）。建议删除该配置文件让插件自动重建默认配置，或参考 config.dsh-theme-synthwave.example.jsonc 修复。") : null, (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.sectionTitle }, "当前背景媒体"), videoPath === "" && imagePaths.length === 0 ? (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.mediaEmpty }, "未设置背景媒体") : (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.mediaList }, videoPath === "" ? null : (0, react.createElement)("div", {
				className: OpenConfigCard_module_css_default.mediaRow,
				key: "video"
			}, (0, react.createElement)("span", { className: OpenConfigCard_module_css_default.mediaKind }, "视频"), (0, react.createElement)("span", { className: OpenConfigCard_module_css_default.mediaName }, videoPath), (0, react.createElement)("button", {
				className: OpenConfigCard_module_css_default.buttonDanger,
				onClick: removeVideo,
				disabled: removing
			}, "移除")), ...imagePaths.map((p, i) => (0, react.createElement)("div", {
				className: OpenConfigCard_module_css_default.mediaRow,
				key: "img-" + i
			}, (0, react.createElement)("span", { className: OpenConfigCard_module_css_default.mediaKind }, "图片"), (0, react.createElement)("span", { className: OpenConfigCard_module_css_default.mediaName }, p), (0, react.createElement)("button", {
				className: OpenConfigCard_module_css_default.buttonDanger,
				onClick: () => removeImage(p),
				disabled: removing
			}, "移除")))), (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.sectionTitle }, "选择背景媒体"), (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.actions }, (0, react.createElement)("button", {
				className: OpenConfigCard_module_css_default.button,
				...
				onClick: cancel,
				disabled: !dirty || saving
			}, "取消")), hint === "" ? null : (0, react.createElement)("div", { className: OpenConfigCard_module_css_default.hint }, hint)) : null);
		}
		//#endregion
...
`;
		...
		function playVideo(bg, video, blurFilter) {
		...
		function imageMerrygo(bg, cfg, blurFilter, disposers) {
			const imgAlpha = typeof cfg.imagesAlpha === "number" ? cfg.imagesAlpha : .85;
			const imgs = cfg.images.map(function(u) {
		...
		function defaultBackground(bg, effect, blurFilter) {
			const BASE_COLORS = [
				"#ff8a3d",
				"#8a4bff",
				"#ff2a6d",
		...
		function injectBackground(cfg) {
			...
			if (b.video) playVideo(bg, b.video, blurFilter);
			else if (b.images && b.images.length) imageMerrygo(bg, b, blurFilter, disposers);
			else defaultBackground(bg, typeof b.defaultEffect === "number" ? b.defaultEffect : 1, blurFilter);
